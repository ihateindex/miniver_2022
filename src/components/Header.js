...
function Header(props) {
    ...
    let movePage = (props) => {
        console.log(props);
        dispatch(changeLoaderInfo({ state: 'loading', page: props }));
        console.log('워크박스 로더 스테이트:', loaderInfo);
    };

    useEffect(() => {
        if (transitionInfo.state === 'start') {
            console.log('워크박스 페이지 트랜지션 스테이트:', transitionInfo);
            navigate(transitionInfo.page);
            dispatch(changeTransitionInfo({ state: 'animate' }));
        }
